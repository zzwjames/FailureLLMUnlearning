...
for name in "${names[@]}"; do
  mkdir -p ./logs/unlearn_bench/People/ga_lora
  id=$name
  echo $id

  PYTHONPATH=./ WANDB_DISABLED=true python src/train_bash.py --stage ga \
    --model_name_or_path /data/zbz5349/ckpt_lr_2e-5_finetune/books/npo_gdr/checkpoint-277 --do_train --save_model \
    --dataset ${id}_Positive --dataset_dir ./data --finetuning_type lora --lora_target q_proj,v_proj \
    --output_dir /data/zbz5349/unlearn_bench/People/${id}/ga_lora/llama3_8b_instruct --overwrite_cache \
    --overwrite_output_dir --cutoff_len 512 --preprocessing_num_workers 4 \
    --per_device_train_batch_size 2 --per_device_eval_batch_size 2 --gradient_accumulation_steps 1 \
    ...
    --learning_rate 1e-5 --num_train_epochs 3.0 --val_size 0.0000001 --plot_loss \
    --output_result_dir ./results/unlearn_bench/People/${id}/ga_lora/llama3_8b_instruct \
    --fp16 --eval_dataset_dir ./data/RWKU/Target \
    --target ${id} 2>&1 | tee ./logs/unlearn_bench/People/ga_lora/llama3_8b_instruct_${id}.log
done