...
for name in "${names[@]}"
do
    mkdir -p ./logs/unlearn_bench/People/dpo_lora
    id=$name
    echo $id
    ...
    --dataset ${id}_Pair --dataset_dir ./data --finetuning_type lora --lora_target q_proj,v_proj \
    --output_dir /data/zbz5349/unlearn_bench/People/${id}/dpo_lora/llama3_8b_instruct --overwrite_cache \
    ...
    --learning_rate 6e-5 --num_train_epochs 3.0 --val_size 0.0000001 --dpo_ftx 0.0 --plot_loss \
    --output_result_dir ./results/unlearn_bench/People/${id}/dpo_lora/llama3_8b_instruct \
    --fp16 --eval_dataset_dir ./data/unlearn_bench/People/ \
    --target ${id} 2>&1 | tee ./logs/unlearn_bench/People/dpo_lora/llama3_8b_instruct_${id}.log
done
